...
void XPT2046_Scan(u8 type)
{
	...
	if(PEN==0)//有按键按下
	{
		if(type)RTouch_Read_XY2(&touch_inst.x,&touch_inst.y);//读取物理坐标
		else if(RTouch_Read_XY2(&touch_inst.x,&touch_inst.y))//读取屏幕坐标
		{
	 		touch_inst.x=xFactor*touch_inst.x+xOffset;     //将结果转换为屏幕坐标
			touch_inst.y=yFactor*touch_inst.y+yOffset;  
	 	} 	
//		Xdown=x;
//		Ydown=y;
    touch_inst.x_down = touch_inst.x;
    touch_inst.y_down = touch_inst.y;
		
//		touch_time++;		   
    touch_inst.touch_time ++;
	}
  else  //建抬起
	{    
		 if(touch_inst.touch_time>2)
		 {
//			 		Xup=x;
//		      Yup=y;	 
          touch_inst.x_up = touch_inst.x;
          touch_inst.y_up = touch_inst.y;
		 }
		touch_inst.touch_time=0;
//		Xdown=0xffff;
//    Ydown=0xffff;
    touch_inst.x_down = touch_inst.x;
    touch_inst.y_down = touch_inst.y;
