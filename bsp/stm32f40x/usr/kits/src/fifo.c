...
{
	uint32_t *mem_ptr;
	mem_ptr = (uint32_t*)rt_malloc(block_size*depth*4);
	if(mem_ptr == RT_NULL)
		return 0;
...
uint32_t	fifo32_push(fifo32_cb_td* fifo_cb, uint32_t* src_addr)
{
	...
	{
		rt_memcpy(fifo_cb->bhead_ptr, src_addr, fifo_cb->block_size*4);
...
uint32_t	fifo32_pop(fifo32_cb_td* fifo_cb, uint32_t* dest_addr)
{
	...
	{
		rt_memcpy(dest_addr, fifo_cb->btail_ptr, fifo_cb->block_size*4);
