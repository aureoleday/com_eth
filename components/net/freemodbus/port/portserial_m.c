...
#define MODBUS_MASTER_RT_CONTROL_PIN_INDEX    91
...
void vMBMasterPortSerialEnable(BOOL xRxEnable, BOOL xTxEnable)
{
    ...
    struct stm32_uart *muart = (struct stm32_uart *) serial->parent.user_data;
    timeout = RT_TICK_PER_SECOND;
    if (xRxEnable)
    {
//        while (!USART_GetFlagStatus(USART1,USART_FLAG_TC))
        while ((!USART_GetFlagStatus(muart->uart_device,USART_FLAG_TC))&&(timeout > 0))
        {
            rt_thread_delay(1);
            timeout--;
        }
        if(timeout <= 0)
            rt_kprintf("error: mbm tx timeout!\n");
        /* enable RX interrupt */
